...
- 导入知识快照时，优先按稳定身份合并，而不是按文件名、显示标题或语言判断是否新建页面
- 项目级页面优先使用 `project_id` 合并；等价版本优先参考 `source_content_hash`；数据对象页、规则页、字典页优先使用条目级 `canonical_key`、物理表名、规则 ID 或命名空间合并
- 当新快照只改变项目中文名、别名、标题、文件名或来源路径，而 `project_id` 与 `source_content_hash` 未变时，应视为同一知识版本的元数据升级，不应重复创建 `tables/`、`rules/`、`concepts/` 或项目入口页
- 如果稳定身份冲突或缺失，应先形成导入评估并暂停确认，不要为了完成导入而猜测合并或新建
